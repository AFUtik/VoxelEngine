...
public:
	...
	inline unsigned char get(int x, int y, int z, int channel) {
		return (map[y * ChunkInfo::DEPTH * ChunkInfo::WIDTH + z * ChunkInfo::WIDTH + x] >> (channel << 2)) & 0xF;
	}

	inline unsigned char getR(int x, int y, int z) {
		return map[y * ChunkInfo::DEPTH * ChunkInfo::WIDTH + z * ChunkInfo::WIDTH + x] & 0xF;
	}

	inline unsigned char getG(int x, int y, int z) {
		return (map[y * ChunkInfo::DEPTH * ChunkInfo::WIDTH + z * ChunkInfo::WIDTH + x] >> 4) & 0xF;
	}

	inline unsigned char getB(int x, int y, int z) {
		return (map[y * ChunkInfo::DEPTH * ChunkInfo::WIDTH + z * ChunkInfo::WIDTH + x] >> 8) & 0xF;
	}

	inline unsigned char getS(int x, int y, int z) {
		return (map[y * ChunkInfo::DEPTH * ChunkInfo::WIDTH + z * ChunkInfo::WIDTH + x] >> 12) & 0xF;
	}
