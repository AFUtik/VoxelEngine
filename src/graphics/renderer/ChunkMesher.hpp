...
class ChunkMesher {
    ...
    inline void updateFaceLight(
        ...
        if constexpr (DIR == direction::UP) {
            consumer.replaceLight(lr0, lg0, lb0, ls0); // 0
            consumer.replaceLight(lr1, lg1, lb1, ls1); // 1
            consumer.replaceLight(lr3, lg3, lb3, ls3); // 3

            consumer.replaceLight(lr1, lg1, lb1, ls1); // 1
            consumer.replaceLight(lr2, lg2, lb2, ls2); // 2
            consumer.replaceLight(lr3, lg3, lb3, ls3); // 3
        }
        else if constexpr (DIR == direction::DOWN) {
            consumer.replaceLight(lr0, lg0, lb0, ls0); // 0
            consumer.replaceLight(lr3, lg3, lb3, ls3); // 1
            consumer.replaceLight(lr1, lg1, lb1, ls1); // 3
...
public:
    ...
    inline void updateCube(Chunk* cur_chunk, VertexConsumer &consumer, int lx, int ly, int lz, uint8_t id)
    {
        float uvsize = 1.0f / 16.0f;
        float u1 = (id % 16) * uvsize;
        float v1 = 1 - ((1 + id / 16) * uvsize);
        ...
        if (!cur_chunk->getBoundBlock(lx, ly+1, lz).id) updateFaceLight<direction::UP>(cur_chunk, consumer, lx, ly, lz, u1, v1, u2, v2);  // UP
        if (!cur_chunk->getBoundBlock(lx, ly-1, lz).id) updateFaceLight<direction::DOWN>(cur_chunk, consumer,  lx, ly, lz, u1, v1, u2, v2); // DOWN
        if (!cur_chunk->getBoundBlock(lx+1, ly, lz).id) updateFaceLight<direction::EAST>(cur_chunk, consumer, lx, ly, lz, u1, v1, u2, v2); // EAST
        if (!cur_chunk->getBoundBlock(lx-1, ly, lz).id) updateFaceLight<direction::WEST>(cur_chunk, consumer,  lx, ly, lz, u1, v1, u2, v2); // WEST
        if (!cur_chunk->getBoundBlock(lx, ly, lz+1).id) updateFaceLight<direction::NORTH>(cur_chunk, consumer,  lx, ly, lz, u1, v1, u2, v2); // NORTH
        if (!cur_chunk->getBoundBlock(lx, ly, lz-1).id) updateFaceLight<direction::SOUTH>(cur_chunk, consumer, lx, ly, lz, u1, v1, u2, v2); // SOUTH
    }
        ...
        for (int y = 0; y < ChunkInfo::HEIGHT; y++) {
            for (int z = 0; z < ChunkInfo::DEPTH; z++) {
                for (int x = 0; x < ChunkInfo::WIDTH; x++) {
                    ...
                    if (!id) continue;
                    updateCube(chunk, consumer, x, y, z, vox.id);
                }
            }
        }
        mesh->update();
